...
const SHORT_STORE: Record<Unit["id"], string> = {
  fatima: "Fátima",
  nacoes: "Nações",
  itacolomi: "Itacolomi",
};
...
function actionText(intent: WhatsAppIntentKey, unitId: Unit["id"]) {
  if (intent === "recipe") return `Receita · ${SHORT_STORE[unitId]}`;
    ...
    <div className={`direct-unit-links ${compact ? "direct-unit-links-compact" : ""} ${className}`.trim()}>
      ...
      <div className="direct-unit-links-grid">
        {units.map((unit: Unit) => {
          ...
          const label = actionText(activeIntent, unit.id);
          ...
          return (
            ...
            >
              ...
              <div className="direct-unit-actions">
                ...
                >
                  <IntentIcon intent={activeIntent} /> {label}
                </a>
              </div>
            </article>
          );
